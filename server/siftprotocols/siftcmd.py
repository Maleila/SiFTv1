...
class SiFT_CMD:
    ...
    def build_command_res(self, cmd_res_struct):
        ...
        # 'success'/'failure' or 'accept'/'reject'
        ...
        if cmd_res_struct['command'] == self.cmd_pwd:
            ...
            if cmd_res_struct['result_1'] == 'failure':
                ...
                cmd_res_str += self.delimiter + \
                    b64encode(cmd_res_struct['result_2'].encode(
                        self.coding)).decode(self.coding)
        ...
        if cmd_res_struct['command'] == self.cmd_pwd:
            ...
            if cmd_res_struct['result_1'] == 'failure':
                ...
                cmd_res_struct['result_2'] = b64decode(
                    cmd_res_fields[3]).decode(self.coding)
    ...
    def send_command(self, cmd_req_struct):
        ...
        try:
            ...
            raise SiFT_CMD_Error(
                'Unable to send command request --> ' + e.err_msg)
        ...
        if msg_type != self.mtp.type_command_res:
            raise SiFT_CMD_Error(
                'Command response expected, but received something else')
        ...
        # chd
            ...
            if dirname == '..':
                if not self.current_dir:  # we are in user root dir
                    ...
                    path = self.server_rootdir + self.user_rootdir + \
                        '/'.join(self.current_dir[:-1])
    ...
    def exec_upl(self, filename):
        if not self.check_fdname(filename):
            raise SiFT_DNL_Error(
                'File name is empty, starts with . or contains unsupported characters')
        else:
            path = self.server_rootdir + \
                self.user_rootdir + '/'.join(self.current_dir)
            if not os.path.exists(path):
                raise SiFT_UPL_Error(
                    'Operation failed due to local error on server')
            else:
                if path[-1] == '/':
                    filepath = path + filename
                else:
