...
class SiFT_DNL:
    ...
    def cancel_download_client(self):
        ...
        try:
            self.mtp.send_msg(self.mtp.type_dnload_req,
                              self.cancel.encode(self.coding))
        except SiFT_MTP_Error as e:
            raise SiFT_DNL_Error(
                'Unable to send download request (cancel) --> ' + e.err_msg)
    ...
    def handle_download_client(self, filepath):
        ...
        try:
            self.mtp.send_msg(self.mtp.type_dnload_req,
                              self.ready.encode(self.coding))
        except SiFT_MTP_Error as e:
            raise SiFT_DNL_Error(
                'Unable to send download request (ready) --> ' + e.err_msg)
        ...
        with open(filepath, 'wb') as f:
            ...
            while not download_complete:
                ...
                try:
                    ...
                    raise SiFT_DNL_Error(
                        'Unable to receive download response --> ' + e.err_msg)
                ...
                if msg_type not in (self.mtp.type_dnload_res_0, self.mtp.type_dnload_res_1):
                    raise SiFT_DNL_Error(
                        'Download response expected, but received something else')
    ...
    def handle_download_server(self, filepath):
        ...
        try:
            ...
            raise SiFT_DNL_Error(
                'Unable to receive download request --> ' + e.err_msg)
        ...
        if msg_type != self.mtp.type_dnload_req:
            raise SiFT_DNL_Error(
                'Download request expected, but received something else')
            ...
            with open(filepath, 'rb') as f:
                ...
                while byte_count == self.size_fragment:
                    ...
                    if byte_count == self.size_fragment:
                        msg_type = self.mtp.type_dnload_res_0
                    else:
                        msg_type = self.mtp.type_dnload_res_1
                    ...
                    try:
                        ...
                        raise SiFT_DNL_Error(
                            'Unable to download file fragment --> ' + e.err_msg)
